...
public:
  ...
  ActionRender(){
    options.add("types", 1,'t',"VMAP2_RENDER", "File with type information "
                "(needed for reading vmap, mp, fig).");
  ...
  std::string get_descr() const override {return "Render image from a vector map";}

  void help_impl(HelpPrinter & pr) override{
    pr.usage("<file1> <file2> ..  <options>");
    pr.head(2, "Options for rendering VMAP");
