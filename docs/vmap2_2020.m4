...
<p>Это старый файл, относящийся к системе фекторных карт в mapsoft2
в период 02.2020 - 11.2022. В данный момент все переделано.
...
должны теряться, но если файл преобразется в другой формат, что все заголовки
теряются. Если читаются несколько файлов MP, то используется заголовок
