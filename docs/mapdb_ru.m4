HEADER(mapdb,`
  Векторные карты в Mapsoft2 (2019-11-03)
')
...
<img src=render_examples/render$1.png align=right>
include(render_examples/render$1.cfg)
</div><br>')
...
<li>Тип объекта, 32-битное целое число. Старший байт - классификации объекта
(0-точка, 1-линия, 2-многоугольник), второй байт не используется, два
младших байта - собственно тип объекта (то же, что тип в MP).
...
TODO: сделать ли идентификатор 64-битным? Или вообще произвольной длины
...
содержащий несколько координат. Вероятно, должна использоваться только
первая точка. Объекты не могут быть пустыми (не содержать координат).
Такой запрет связан с невозможностью разместить такой объект в базе
geohash.db.
...
<pre><tt>
    (point|line|area):&lt;tnum&gt; &lt;feature&gt; &lt;options&gt; ...
    + &lt;feature&gt; &lt;options&gt; ...
    + &lt;feature&gt; &lt;options&gt; ...
    ...
    map &lt;feature&gt; &lt;options&gt; ...
...
area. Цвет всегда задается в виде 32-битного числа с прозрачностью
(0xFF000000 - черный, 0xFFFF0000 - красный, 0x80FFFFFF - белый
полупрозрачный и т.п.)
...
area, map. Картинка - в любом растровом формате, поддержтиваемом
...
выравнивание будет сделано по левому-нижнему углу. TODO: векторные
форматы типа svg, pdf...
...
окончание линии. Применимо к шагам line, area, используется
совместно со свойством stroke. По умолчанию - round.

RENDER_EXAMPLE(04)

<li><tt> join round|miter </tt> -- Описывает, как рисовать
стыки сегментов линий. Применимо к шагам line, area, используется
совместно со свойством stroke. По умолчанию - round.
...
smooth и т.д.). Координты ориентированы по направлению объекта: x вдоль
линии, y - перпендикулярно, вправо от нее.
...
<li><tt>draw_pos (point|begin|end|dist|adist) </tt> -- Место рисования
...
adist -- периодически вдоль объекта, примерно на заданном расcтоянии друг
от друга, так, чтобы расположение объектов было симметрично относительно
концов линии.

<li><tt> draw_dist &lt;dist&gt; [&lt;dist0&gt;] </tt> -- Расстояния,
использующиеся в <tt>draw_pos dist/adist</tt>. dist -- расстояние между
соседними местами рисования; dist0 -- расстояние от первой точки до
первого места рисования (а для свойства draw_pos adist - и расстояние от
последнего места рисования до последней точки объекта) dist/adist
position
...
<li>Если шаг имеет тип point, line, area, то выбираются соответствующие
объекты в нужном диапазоне.

<li>Если присутствуют свойства stroke, fill, patt, то строится "путь"
рисования (path).

<li>Настраивается оператор рисования (свойство operator).
...
<!--#################################################################-->
<hr><h3>Наклон и масштаб объектов и подписей (пока не реализовано)</h3>

<p>Объекты и подписи имеют параметр "наклон" (angle). Наклон
отсчитывается от географического севера, значение по умолчанию - 0.
Правило рисования для данного типа объекта или подписей может
выставить фиксированный наклон относительно верха карты
Кроме того, правило рисования rotate_to выставляет наклон объекта по
заданному другому объекту.
