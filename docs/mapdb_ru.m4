...
определенным типом и координатами. Тим содержит классификацию объекта
(точка, линия, площадь, такст) и номер (16-битное число). Объекты могут
также содержать о том, как следует показывать данный объект (угол
наклона, увеличение) и связи с другими объектами (например, какие
текстовые подписи относятся к данному объекту).

<p>Такой набор объектов не содержит информации о типах объектов: как их
надо рисовать, какие между ними существуют отношения и т.п., а также о
растровом представлении этой карты: ее системе координат, границе,
масштабе, сетке и т.п.
