...
<p>Привязка карты устанавливается через конфигурационный файл (команда <tt>set_ref</tt>),
из файла или по номенклатурному листу. При этом также устанавливается граница (которую
потом можно нарисовать командой <tt>brd</tt>). Также, границу можно изменить командой
...
ничто не мешает нарисовать карту в любой системе. Например, при рисовании
карты через ms2mapdb render используется связка функций
<tt>geo_mkref</tt> (задание привязки с помощью набора параметров) и
<tt>write_geoimg</tt> - рисование картинки в соответствии с привязкой. По
умолчанию используется собственная привязка карты, но ее можно поменять с
помощью параметров geo_mkref. Отдельно можно поменять границу с помощью
переметров border_wgs или border_file. Можно использовать параметр
--tmap, чтобы сгенерировать набор плиток (они, разумеется, рисуются в
своей собственной системе координат).
...
множитель для всех таких размеров.
