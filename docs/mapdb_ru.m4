...
единицах 1/1000 градуса. Запакован с тэгом "angl". Имеет смысл только для
точечных и текстовых объектов. В данный момент наклон считается от "верха
карты". То есть, если рисовать карту в разных проекциях, объекты с
наклоном 0 всегда будут ориентированы вверх. Возможно, следует
отсчитывать наклон от географического севера. В даный момент отсутствие
информации о наклоне объекта эквивалентно наклону 0. Возможно, следует
различать эти значения.

<li>TODO: масштаб объекта (для точек и текста).

<li>TODO: выравнивание (длят текста).
...
текстовый объект, содержащий несколько координат. Вероятно, должна
использоваться только первая точка. Объекты не могут быть пустыми (не
содержать координат). Такой запрет связан с невозможностью разместить
такой объект в базе geohash.db.
...
TODO: сейчас поворачиваются только картинки. Сделать, чтобы точка,
нариованная свойством lines/circles тоже поворачивалась. Ну и вообще,
повороты объектов пока не готовы...
