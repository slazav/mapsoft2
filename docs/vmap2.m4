...
строки _GEOHASH_, значение - id объекта в основной базе. Записи с
...
<p>См. <tt>vmap2/vmap2types.h</tt> и пример файла <tt>vmap_data/types.cfg</tt>
...
<li><tt>float label_def_align</tt> -- Выравнивание подписи при создании.
При создании подписи ее параметр align устанавливается в это значение.
Например SW, S, SE (возможно 9 значений, но при использовании в fig все
преобразуется в эти три).
...
<li><tt>int label_maxnum</tt> -- Максимальное количество подписей,
которое может быть прицеплено к объекту данного типа. Если значение
отрицательно (например -1) то количество подписей произвольно. Если
значение -2 (это значение по умолчанию) то используется 0 для текстовых
объектов, 1 для точек, -2 для остального.
...
<p>Обратите внимание, что размер шрифта в render.cfg - в пикселах (как в libcairo),
для "естественной" привязки карты. А в параметре fig_mask - в типографских точках
на картинке xfig. И шрифты и их размеры могут быть разными. Рекомендуется
подобрать такое значение в fig_mask, чтобы подписи примерно совпадали.
Например, если "естественное" разрешение карты 150dpi, то шрифт 10px будет примерно
соответствовать размеру шрифта в xfig: 10px / 150px/in * 72pt/in = 4.8pt.
...
mapsoft1. До сих пор все исходники векторных карт хранятся в этом
формате, хотя уже давно надо бы их преобразовать. Поддержка формата - в
<tt>vmap/</tt>, преобразование в VMAP2 - в <tt>vmap2/vmap2io_vmap.cpp</tt>.

<p>Основные отличия: в vmap2 подписи всегда отвязаны от объектов, в vmap
была возможность хранить их как в объектах, так и отдельно, причем в
реальности все подписи хранились в объектах. Размер подписей указывался
в виде числе-добавки к номинальному размеру: -1, +2 и т.п. Это, конечно,
очень неудобно, когда подпись отвязана от объекта, а "естественный
размер" определяется только на этапе рендера.
...
<li> Преобразуются только подписи, привязанные к объектам, и только если
в файле typeinfo указан параметр label_type != -1. В формате VMAP были
придусмотрены и подписи с "мягкой" привязкой. Но реально в файл они не
записывались и тут не поддерживаются.
...
"Польский формат", использующийся для преобразования карт в векторные
...
WWW(`https://github.com/slazav/mapsoft2-libs/tree/master/mp#readme')
...
<li>Данные берутся из самого нижнего непустого слоя (то есть, если есть
...
<!--##################################################################-->
<hr><h4>формат OSM XML</h4>

<p>В библиотеке mapsoft2 имеется небольшой модуль для чтения формата
OSM XML. Объекты типа node читаются в ассоциативный массив id - координаты,
кроме того, читаются объекты point (node с непустыми тэгами), way и
relation.

<p>Поддерживается преобразование OSM XML в другие векторные форматы с
помощью программ ms2vmap, ms2vmapdb. Для этого используется отдельный
конфигурационный файл, который задается параметром --osm_conf. Формат
файла пока не устоялся, сейчас читаются строчки типа "<tt>(point|way)
&lt;options&rt; &lt;type1&rt; [&lt;type2&rt;]</tt>". Здесь options - json
объект, поля которого должны совпасть с тегами объекта, type1 - тип в
который преобразуется объект, type2 (задается только для линий) -
точечный тип в который преобразуется объект, если он слишком мал.
Предельный размер задается командой "<tt>set min_size &lt;value&rt;</tt>"
в метрах и действет на все последующие записи в файле. Значение по
умолчанию - 10м.
...
<p>Смотрите также файл <tt>vmap_data/osm.cfg</tt>.

<!--##################################################################-->
<hr><h4>формат Shape</h4>

<p>В mapsoft2 есть возможность читать и писать файлы Shape с помощью
библиотеки _SHAPELIB_. Взаимодействие с форматом vmap2 пока не сделано, но,
кажется, при необходимости это сделать несложно.

<!--##################################################################-->
<hr><h4>формат OCAD</h4>

<p>В mapsoft2 есть некоторая поддерсжка формате OCAD, но она пока не
доведена до конца и не поддерживает взаимодействие с форматом vmap2.
