...
Возможно, следует ее переделать в обычную многосегментную линию...
...
<p>Для треков и маршрутов (в формате gpx они устроены похоже)
поддерживаются все основные поля (имя, комментарий), следующие
дополнительные поля читаются в виде дополнительных параметров с префиксом
"gpx_" и при записи в GPX не должны потеряться:
...
<p>При этом сам формат GeoJSON поддерживается не полностью:
<ul>
...
по имени файла (без расширения .wpt).
...
<li>Поддержка точек привязок, записанных в координатах WGS84 или в grid координатах.
    Формат поддерживает до 30 точек привязки. В интерфейсе OziExplorer поддерживалось 4 точки,
...
<li>TB(`--trk_name <arg>'), TB(`--maplist_name <arg>'), TB(`--wptlist_name <arg>')
 -- Поменять имя трека, списка карт или списка точек. По умолчанию меняются имена
всех трекав или списков, но можно указать конкретный номер с помощью
параметра TB(`--*_num').

<li>TB(`--trk_comm <arg>'), TB(`--maplist_comm <arg>'), TB(`--wptlist_comm <arg>')
 -- То же для комментариев.

<li>TB(`--trk_num <arg>'), TB(`--maplist_num <arg>'), TB(`--wptlist_num <arg>')
-- Номер трека, списка карт или списка точек, на которые будут действовать параметры
TB(`--*_name') и (`--*_comm'). Нумерация начинается с 1, 0 (значение по
умолчанию) означает все объекты.
...
ПРограмма может нарисовать геоданные (и не только) в виде картинке в
растровом формате (jpeg, png, gif, tiff) или форматах pdf, ps, svg.
